...
const BlogPost = () => {
        ...
        return (
            <div className="min-h-screen bg-primary flex items-center justify-center text-center px-4">
                <div className="max-w-md bg-primary-light p-10 rounded-2xl border border-white/10 shadow-xl">
                    ...
                    <Link to="/blog" className="btn-primary w-full justify-center">Back to Knowledge Hub</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        ...
        >
                    ...
                    <div className="flex flex-wrap items-center gap-8 pt-8 border-t border-white/10">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-white font-medium">{post.date}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* Related Posts Section - Redesigned */}
            <section className="py-24 bg-primary-light relative overflow-hidden">
                <div className="max-w-6xl mx-auto px-4 lg:px-0 relative z-10">
                    <div className="flex justify-between items-end mb-12">
                        <div>
                            <span className="text-accent-teal font-bold uppercase tracking-widest text-xs mb-3 block">Keep Reading</span>
                            <h2 className="text-3xl md:text-4xl font-display font-bold text-white">More Professional Insights</h2>
                        </div>
                        <Link to="/blog" className="hidden md:flex items-center gap-2 text-white border-b border-accent-gold pb-1 font-bold text-sm hover:text-accent-gold transition-colors">
                            All Articles
                        </Link>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        {relatedPosts.map(related => (
                            <Link key={related.id} to={`/blog/${related.slug}`} className="group flex flex-col h-full">
                                <div className="aspect-[16/9] rounded-2xl overflow-hidden mb-6 border border-white/10 shadow-lg relative">
                                ...
                                <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-accent-gold mb-3">
                                    <span>{related.category}</span>
                                    <span className="w-1 h-1 rounded-full bg-slate-600"></span>
                                    <span>{related.readTime}</span>
                                </div>
                                <h3 className="text-xl md:text-2xl font-bold text-white group-hover:text-accent-teal transition-colors leading-tight mb-4">
                                    {related.title}
                                </h3>
                                <p className="text-slate-400 text-sm line-clamp-2 mb-6 flex-grow">
                                    {related.excerpt}
                                </p>
                                <div className="mt-auto flex items-center gap-2 text-white text-xs font-bold group-hover:gap-4 transition-all">
                                    READ ARTICLE <ArrowRight className="h-4 w-4 text-accent-teal" />
                                </div>
                            </Link>
                        ))}
                    </div>
